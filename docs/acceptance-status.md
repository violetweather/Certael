...
| Signed version lifecycle | In progress | Cross-language canonical compatibility policy, offline Ed25519 trust ring, rollback floor, support/deprecation/update/revocation decisions, Core admission gate, build/SDK binding, Agent TUF staging, status API/CLI/metrics, and migration guide | Exercise offline production key ceremony, rotation, expiry outage, withdrawal, and platform update approval in protected CI |
...
| Equivalent Godot, Unity, Unreal SDKs | In progress | Typed binary adapters, versioned C ABI, native verifier, Godot/Unity Agent lifecycle relays, Blueprint-accessible Unreal subsystem, typed Unreal results/events, non-blocking Agent challenge node, Unity .NET Standard compatibility build, local Godot 4.7-compatible macOS arm64 native build, installer detection, and packaging workflow | Pinned-editor Unreal 5.8 and Unity IL2CPP execution, executable Blueprint-first Unreal sample, executable samples for the other engines, and cross-platform Agent relay E2E |
